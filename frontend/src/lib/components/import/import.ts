...
@WithRender
@Component({
	components: { Icon, FileResults, Portal }
})
export default class Import extends Vue {
	...
	handleOpenSelection(): void {
		for (let i = 0; i < this.layerIds.length; i++) {
			if (this.mapContext.selection.some((item) => item.type == "searchResult" && item.layerId == this.layerIds[i])) {
				this.mapContext.$emit("fm-search-box-show-tab", `fm${this.context.id}-import-tab-${i}`);
				break;
			}
		}
	}
	...
	async importFiles(files: FileList | undefined): Promise<void> {
		this.$bvToast.hide(`fm${this.context.id}-import-error`);
		...
		try {
			...
			if (result.features.length == 0 && result.errors)
				...
				setTimeout(() => {
					this.mapContext.$emit("fm-search-box-show-tab", `fm${this.context.id}-import-tab-${this.files.length - 1}`);
				}, 0);
			}
		} catch (err) {
