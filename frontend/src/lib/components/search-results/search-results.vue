<script setup lang="ts">
	...
	export default class SearchResults extends Vue {
		...
		handleOpen(result: SearchResult | FileResult | FindOnMapResult, event: MouseEvent): void {
			...
			setTimeout(async () => {
				if (isMapResult(result)) {
					...
					this.mapContext.$emit("fm-search-box-show-tab", `fm${this.context.id}-${result.kind}-info-tab`, false);
				} else
					this.activeTab = 1;
			}, 0);
		}
		...
		async _addToMap(data: Array<{ result: SearchResult | FileResult; type: Type }>): Promise<boolean> {
			this.$bvToast.hide(`fm${this.context.id}-search-result-info-add-error`);
			this.isAdding = true;
