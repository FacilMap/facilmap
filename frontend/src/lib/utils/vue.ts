 ...
 */
export function vDirectiveWithModifiers<M extends string, D extends Directive<any, any, M, any>>(directive: D, modifiers: Ref<Record<M, boolean>>): D {
	function adjustBinding(binding: DirectiveBinding): DirectiveBinding {
		return {
			...binding,
			modifiers: new Proxy({}, {
				get(_, prop) {
					return Object.hasOwn(binding.modifiers, prop) ? binding.modifiers[prop as M] : modifiers.value[prop as M];
				},
				ownKeys() {
	...
	if (typeof directive === "function") {
		return ((el, binding, ...rest) => {
			directive(el, adjustBinding(binding), ...rest);
		}) as D;
	} else {
		return Object.fromEntries(["created", "beforeMount", "mounted", "beforeUpdate", "updated", "beforeUnmount", "unmounted"].flatMap((k) => {
			if (k in directive) {
				return [[k, (el: any, binding: DirectiveBinding, ...rest: any[]) => {
					(directive as any)[k](el, adjustBinding(binding), ...rest);
				}]];
			} else {
				return [];
			}
