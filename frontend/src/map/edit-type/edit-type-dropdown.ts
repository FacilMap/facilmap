...
export default class EditTypeDropdown extends Vue {
	...
	save(): void {
		this.$bvToast.hide("fm-edit-type-dropdown-error");

		const idx = this.type.fields.indexOf(this.field);
		if (idx === -1)
			showErrorToast(this, "fm-edit-type-dropdown-error", "Error updating field", new Error("The field cannot be found on the type anymore."));
		else {
			Vue.nextTick(() => {
