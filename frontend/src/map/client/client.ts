...
export class ClientProvider extends Vue {
    ...
    async connect(): Promise<void> {
        ...
        this.$bvToast.hide("fm-client-connecting");
        this.$bvToast.hide("fm-client-error");
        this.$bvToast.hide("fm-client-deleted");
        this.createId = null;
        if (this.context.activePadId)
            showToast(this, "fm-client-connecting", "Loading", "Loading map…", { spinner: true });
        else
            showToast(this, "fm-client-connecting", "Connecting", "Connecting to server…", { spinner: true });
        ...
        client.on("deletePad", () => {
            showToast(this, "fm-client-deleted", "Map deleted", "This map has been deleted.", {
                variant: "danger",
        ...
        if (client.serverError?.message?.includes("does not exist") && this.context.interactive) {
            ...
            showErrorToast(this, "fm-client-error", "Error opening map", client.serverError, {
                noCloseButton: true,
                actions: this.context.interactive ? [
                    {
                        label: "Close map",
