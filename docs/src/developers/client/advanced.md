...
* For the REST API, you can use Basic HTTP Authentication, providing any username and the map password. You can send the base64-encoded string `:${password}` as the `Authorization` header. The string is expected to be encoded in UTF-8. In JavaScript, you can create the string by calling ``btoa(unescape(encodeURIComponent(`:${password}`)))``.
