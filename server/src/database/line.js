...
module.exports = function(Database) {
	...
	Object.assign(Database.prototype, {
		...
		getPadLinesWithPoints(padId, bboxWithZoom) {
			return filterStreamPromise(this.getPadLines(padId), (data) => {
		...
		getLineTemplate(padId, data) {
			return promiseAuto({
				lineTemplate: () => {
					var ret = JSON.parse(JSON.stringify(this._conn.model("Line").build({ ...data, padId: padId })));
					ret.data = data.data || { };
					return ret;
				},
		...
		getLinePointsForPad(padId, bboxWithZoom) {
			return filterStreamPromise(this.getPadLines(padId, "id"), (line) => {
		...
		async _calculateRouting(line, trackPointsFromRoute) {
			...
			if(trackPointsFromRoute) {
				...
				line.distance = calculateDistance(line.trackPoints);
				line.time = null;
				line.extraInfo = {};
				...
				line.distance = calculateDistance(line.routePoints);
				line.time = null;
				line.extraInfo = {};

				trackPoints = [ ];
				for(var i=0; i<line.routePoints.length; i++) {
					trackPoints.push({ ...line.routePoints[i], zoom: 1, idx: i });
				}
			}

			Object.assign(line, calculateBbox(trackPoints));
