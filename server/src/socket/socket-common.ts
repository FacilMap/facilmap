...
import { type EventHandler, type EventName, SocketVersion, type SocketApi } from "facilmap-types";
...
export type SocketHandlers<V extends SocketVersion> = {
	[K in keyof SocketApi<V, true>]: SocketApi<V, true>[K] extends (...args: infer Args) => Promise<infer Ret> ? (...args: Args) => FixedNullResponse<Ret> | PromiseLike<FixedNullResponse<Ret>> : never;
};
