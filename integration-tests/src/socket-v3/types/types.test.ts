...
import { ApiVersion, CRU, SocketVersion, type Type } from "facilmap-types";
import { cloneDeep } from "lodash-es";

describe.for([
	{ label: "Socket API", useSocket: true },
	{ label: "REST API", useSocket: false }
])("Type tests ($label)", ({ useSocket }) => {
	...
	test("Create type (marker, default settings)", async () => {
		...
		await createTemporaryMap(storage1, { createDefaultTypes: false }, async (createMapData, mapData) => {
			...
			const typeResult = await (restClient ?? storage1.client).createType(mapData.adminId, type);
	...
	test("Create type (custom settings)", async () => {
		...
		await createTemporaryMap(storage, { createDefaultTypes: false }, async (createMapData, mapData) => {
			...
			const typeResult = await (restClient ?? storage.client).createType(mapData.adminId, type);
	...
	test("Update type", async () => {
		...
		await createTemporaryMap(storage1, { createDefaultTypes: false }, async (createMapData, mapData) => {
			const createdType = await (restClient ?? storage1.client).createType(mapData.adminId, {
				name: "Test type",
				type: "marker"
			});
			...
			const typeResult = await (restClient ?? storage1.client).updateType(mapData.adminId, createdType.id, update);
	...
	test("Delete type", async () => {
		...
		await createTemporaryMap(storage1, { createDefaultTypes: false }, async (createMapData, mapData) => {
			const type = await (restClient ?? storage1.client).createType(mapData.adminId, {
				name: "Test type",
				type: "marker"
			});
			...
			await (restClient ?? storage1.client).deleteType(mapData.adminId, type.id);
