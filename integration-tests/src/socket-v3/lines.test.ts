...
import { CRU, type Line, type FindOnMapLine, type LineWithTrackPoints, type LinePoints, ApiVersion, SocketVersion } from "facilmap-types";
import { cloneDeep, omit } from "lodash-es";

describe.for([
	{ label: "Socket API", useSocket: true },
	{ label: "REST API", useSocket: false }
])("Line tests ($label)", ({ useSocket }) => {
	...
	test("Create line (using default values)", async () => {
		...
		await createTemporaryMap(storage1, {}, async (createMapData, mapData) => {
			...
			const line = await (restClient ?? storage1.client).createLine(mapData.adminId, {
				routePoints: [
					{ lat: 6, lon: 6 },
					{ lat: 14, lon: 14 }
				],
	...
	test("Create line (using custom values)", async () => {
		...
		await createTemporaryMap(storage, {}, async (createMapData, mapData) => {
			...
			const line = await (restClient ?? storage.client).createLine(mapData.adminId, data);
	...
	test("Edit line", async () => {
		...
		await createTemporaryMap(storage1, {}, async (createMapData, mapData) => {
			...
			const createdLine = await (restClient ?? storage1.client).createLine(mapData.adminId, {
				routePoints: [
					{ lat: 6, lon: 6 },
					{ lat: 14, lon: 14 }
				],
			...
			const secondType = await (restClient ?? storage1.client).createType(mapData.adminId, {
				type: "line",
				name: "Second type"
			});
			...
			const line = await (restClient ?? storage1.client).updateLine(mapData.adminId, createdLine.id, newData);
	...
	test("Delete line", async () => {
		...
		await createTemporaryMap(storage1, {}, async (createMapData, mapData) => {
			...
			await (restClient ?? storage1.client).deleteLine(mapData.adminId, createdLine.id);
	...
	test("Find line", async () => {
		...
		await createTemporaryMap(storage1, {}, async (createMapData, mapData) => {
			...
			const marker = await (restClient ?? storage1.client).createLine(mapData.adminId, {
				name: "Line test",
				routePoints: [
					{ lat: 6, lon: 6 },
					{ lat: 14, lon: 14 }
			...
			expect(await (restClient ?? storage2.client).findOnMap(mapData.readId, "Test")).toEqual([{ ...expectedResult, similarity: 0.4 }]);
			expect(await (restClient ?? storage2.client).findOnMap(mapData.readId, "T_st")).toEqual([{ ...expectedResult, similarity: 0.2 }]);
			expect(await (restClient ?? storage2.client).findOnMap(mapData.readId, "L%e")).toEqual([{ ...expectedResult, similarity: 0 }]);
			expect(await (restClient ?? storage2.client).findOnMap(mapData.readId, "Bla")).toEqual([]);
		});
	});
	...
	test("Try to create line with marker type", async () => {
		...
		await createTemporaryMap(storage, {}, async (createMapData, mapData) => {
			...
			await expect(async () => {
				await (restClient ?? storage.client).createLine(mapData.adminId, {
					routePoints: [
						{ lat: 6, lon: 6 },
						{ lat: 14, lon: 14 }
					],
			...
			const storage3 = await openClientStorage(createMapData.adminId, SocketVersion.V3);
	...
	test("Try to update line with line type", async () => {
		...
		await createTemporaryMap(storage, {}, async (createMapData, mapData) => {
			...
			await expect(async () => {
				await (restClient ?? storage.client).updateLine(mapData.adminId, line.id, {
	...
	test("Try to create line with line type from other map", async () => {
		...
		await createTemporaryMap(storage1, {}, async (createMapData, mapData) => {
			await createTemporaryMap(storage2, {}, async (createMapData2, mapData2) => {
				...
				await expect(async () => {
					await (restClient ?? storage1.client).createLine(mapData.adminId, {
						routePoints: [
							{ lat: 6, lon: 6 },
							{ lat: 14, lon: 14 }
						],
	...
	test("Try to update line with line type from other map", async () => {
		...
		await createTemporaryMap(storage1, {}, async (createMapData, mapData) => {
			await createTemporaryMap(storage2, {}, async (createMapData2, mapData2) => {
				...
				await expect(async () => {
					await (restClient ?? storage1.client).updateLine(mapData.adminId, line.id, {
						typeId: lineType2.id
					});
				}).rejects.toThrowError("could not be found");
	...
	test("Export line", async () => {
		...
		await createTemporaryMap(storage, {}, async (createMapData, mapData) => {
			...
			const exportedTrk = await new Response((await (restClient ?? storage.client).exportLine(mapData.adminId, line.id, { format: "gpx-trk" })).data).text();
...
<gpx xmlns="http://www.topografix.com/GPX/1/1" creator="FacilMap" version="1.1" xmlns:osmand="https://osmand.net" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.topografix.com/GPX/1/1 http://www.topografix.com/GPX/1/1/gpx.xsd">
	...
	<trk>
		...
		<trkseg>
			...
			const exportedRte = await new Response((await (restClient ?? storage.client).exportLine(mapData.adminId, line.id, { format: "gpx-rte" })).data).text();
