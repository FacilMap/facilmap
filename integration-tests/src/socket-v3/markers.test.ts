...
import { CRU, type Marker, type FindOnMapMarker, SocketVersion, ApiVersion } from "facilmap-types";
import { cloneDeep } from "lodash-es";

describe.for([
	{ label: "Socket API", useSocket: true },
	{ label: "REST API", useSocket: false }
])("Marker tests ($label)", ({ useSocket }) => {
	...
	test("Create marker (using default values)", async () => {
		...
		await createTemporaryMap(storage1, {}, async (createMapData, mapData) => {
			...
			const marker = await (restClient ?? storage1.client).createMarker(mapData.adminId, {
				lat: 10,
				lon: 10,
	...
	test("Create marker (using custom values)", async () => {
		...
		await createTemporaryMap(storage, {}, async (createMapData, mapData) => {
			...
			const marker = await (restClient ?? storage.client).createMarker(mapData.adminId, data);
	...
	test("Edit marker", async () => {
		...
		await createTemporaryMap(storage1, {}, async (createMapData, mapData) => {
			...
			const createdMarker = await (restClient ?? storage1.client).createMarker(mapData.adminId, {
				lat: 10,
				lon: 10,
				typeId: markerType.id,
				ele: null
			...
			const marker = await (restClient ?? storage1.client).updateMarker(mapData.adminId, createdMarker.id, newData);
	...
	test("Delete marker", async () => {
		...
		await createTemporaryMap(storage1, {}, async (createMapData, mapData) => {
			...
			await (restClient ?? storage1.client).deleteMarker(mapData.adminId, createdMarker.id);
	...
	test("Get marker", async () => {
		...
		await createTemporaryMap(storage1, {}, async (createMapData, mapData) => {
			...
			expect(await (restClient ?? storage2.client).getMarker(mapData.readId, marker.id)).toEqual(expectedMarker);
		});
	});
	...
	test("Find marker", async () => {
		...
		await createTemporaryMap(storage1, {}, async (createMapData, mapData) => {
			...
			expect(await (restClient ?? storage2.client).findOnMap(mapData.readId, "Test")).toEqual([{ ...expectedResult, similarity: 0.3333333333333333 }]);
			expect(await (restClient ?? storage2.client).findOnMap(mapData.readId, "T_st")).toEqual([{ ...expectedResult, similarity: 0.16666666666666666 }]);
			expect(await (restClient ?? storage2.client).findOnMap(mapData.readId, "M%r")).toEqual([{ ...expectedResult, similarity: 0 }]);
	...
	test("Try to create marker with line type", async () => {
		...
		await createTemporaryMap(storage, {}, async (createMapData, mapData) => {
			...
			await expect(async () => {
				await (restClient ?? storage.client).createMarker(mapData.adminId, {
					lat: 10,
					lon: 10,
					typeId: lineType.id
				});
	...
	test("Try to update marker with line type", async () => {
		...
		await createTemporaryMap(storage, {}, async (createMapData, mapData) => {
			...
			await expect(async () => {
				await (restClient ?? storage.client).updateMarker(mapData.adminId, marker.id, {
					typeId: lineType.id
				});
			}).rejects.toThrowError("Cannot use line type for marker");
	...
	test("Try to create marker with marker type from other map", async () => {
		...
		await createTemporaryMap(storage1, {}, async (createMapData, mapData) => {
			await createTemporaryMap(storage2, {}, async (createMapData2, mapData2) => {
				...
				await expect(async () => {
					await (restClient ?? storage1.client).createMarker(mapData.adminId, {
						lat: 10,
						lon: 10,
	...
	test("Try to update marker with marker type from other map", async () => {
		...
		await createTemporaryMap(storage1, {}, async (createMapData, mapData) => {
			await createTemporaryMap(storage2, {}, async (createMapData2, mapData2) => {
				...
				await expect(async () => {
					await (restClient ?? storage1.client).updateMarker(mapData.adminId, marker.id, {
						typeId: markerType2.id
					});
				}).rejects.toThrowError("could not be found");
