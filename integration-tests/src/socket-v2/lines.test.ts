...
import { CRU, type LegacyV2LinePointsEvent, type FindOnMapLine, type ID, SocketVersion, type LegacyV2Line, type TrackPoints } from "facilmap-types";
...
test("Create line (using default values)", async () => {
	...
	await createTemporaryMapV2(client1, {}, async (createMapData, mapData) => {
		...
		const expectedLinePointsEvent = {
			...
			reset: true
		} satisfies LegacyV2LinePointsEvent;
		...
		} satisfies LegacyV2Line & { trackPoints: TrackPoints };
...
test("Create line (using custom values)", async () => {
	...
	await createTemporaryMapV2(client, {}, async (createMapData, mapData) => {
		...
		const data: LegacyV2Line<CRU.CREATE> = {
			routePoints: [
				{ lat: 6, lon: 6 },
				{ lat: 12, lon: 12 }
			],
...
test("Edit line", async () => {
	...
	await createTemporaryMapV2(client1, {}, async (createMapData, mapData) => {
		...
		} satisfies LegacyV2Line<CRU.UPDATE> & { id: ID };
...
test("Try to create line with marker type", async () => {
	...
	await createTemporaryMapV2(client, {}, async (createMapData) => {
		...
		const client3 = await openClientV2(createMapData.adminId, SocketVersion.V2);
...
test("Try to create line with line type from other map", async () => {
	...
	await createTemporaryMapV2(client1, {}, async (createMapData) => {
		await createTemporaryMapV2(client2, {}, async () => {
