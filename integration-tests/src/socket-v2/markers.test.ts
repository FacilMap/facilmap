...
import { CRU, type ID, SocketVersion, type LegacyV2Marker, type LegacyV2FindOnMapMarker } from "facilmap-types";
...
test("Create marker (using custom values)", async () => {
	...
	await createTemporaryMapV2(client, {}, async (createMapData, mapData) => {
		...
		const data: LegacyV2Marker<CRU.CREATE> = {
			lat: 10,
			lon: 10,
			typeId: markerType.id,
			name: "Test marker",
...
test("Edit marker", async () => {
	...
	await createTemporaryMapV2(client1, {}, async (createMapData, mapData) => {
		...
		const newData: LegacyV2Marker<CRU.UPDATE> & { id: ID } = {
			id: createdMarker.id,
			lat: 10,
			lon: 10,
...
test("Find marker", async () => {
	...
	await createTemporaryMapV2(client1, {}, async (createMapData, mapData) => {
		...
		const expectedResult: LegacyV2FindOnMapMarker = {
			id: marker.id,
			kind: "marker",
			similarity: 1,
			lat: 10,
